...
export default {
  ...
  computed: {
    ...
    zongjia:{
      get: function () {
        return 1*this.ruleForm.zulinshijia*this.ruleForm.zulinshizhang
      }
    },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `qicaiyuding/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.qicaiyudingCrossAddOrUpdateFlag = false;
						 this.parent.search();
